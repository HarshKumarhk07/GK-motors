    ...
    <div style={{ minHeight: '100vh', background: '#FFFFFF', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '0.5rem 1rem' }}>
      ...
      <div style={{ width: '100%', maxWidth: 460 }}>
          ...
          <form onSubmit={handleSubmit(onSubmit)}>
            {[
              ...
              <div key={name} className="form-input-wrapper" style={{ marginBottom: '0.8rem' }}>
                ...
                {errors[name] && <p style={{ color: '#1567D3', fontSize: '0.82rem', marginTop: '0.4rem', fontWeight: 700 }}>{errors[name].message}</p>}
              </div>
            ))}
            ...
            <div className="form-input-wrapper" style={{ marginBottom: '1rem' }}>
              ...
              {errors.password && <p style={{ color: '#1567D3', fontSize: '0.82rem', marginTop: '0.4rem', fontWeight: 700 }}>{errors.password.message}</p>}
            </div>
 
            <p className="terms-text" style={{ color: '#94A3B8', fontSize: '0.78rem', marginBottom: '1.2rem', lineHeight: 1.4, fontWeight: 500 }}>
              By creating an account, you agree to our{' '}
              <Link to="/terms" style={{ color: '#1567D3', textDecoration: 'none', fontWeight: 800 }}>Terms</Link> and{' '}
              <Link to="/privacy" style={{ color: '#1567D3', textDecoration: 'none', fontWeight: 800 }}>Privacy</Link>.
            </p>
 
            <button type="submit" className="btn-primary submit-btn" 
              style={{ width: '100%', justifyContent: 'center', padding: '1rem', fontSize: '1rem', fontWeight: 800, borderRadius: '16px', background: '#1567D3', border: 'none', color: 'white', cursor: 'pointer', fontFamily: "'Space Grotesk', sans-serif", letterSpacing: '0.1em', boxShadow: '0 12px 30px rgba(21, 103, 211, 0.25)', transition: 'all 0.3s' }} 
              disabled={loading}
        ...
        <p className="signin-text" style={{ textAlign: 'center', color: '#64748B', marginTop: '1.2rem', fontSize: '0.9rem', fontWeight: 600 }}>
          ...
          <Link to={redirectTo !== '/' ? `/login?redirect=${encodeURIComponent(redirectTo)}` : "/login"} style={{ color: '#1567D3', textDecoration: 'none', fontWeight: 800 }}>Sign in</Link>
        </p>
      </div>
    </div>
  );
