    ...
    <div style={{ minHeight: '100vh', background: '#FFFFFF' }}>
      ...
      <div style={{ background: '#F9F9F9', borderBottom: '1px solid #EEE', padding: '0.8rem 0' }}>
        <div className="max-w-6xl mx-auto px-4 flex items-center gap-2" style={{ fontSize: '0.9rem', color: '#666', fontWeight: 600 }}>
          ...
          <span>/</span>
          <span style={{ color: '#1567D3', fontWeight: 900, textTransform: 'uppercase', letterSpacing: '0.05em' }}>{part.name}</span>
        </div>
      </div>
        ...
        <div className="animate-fadeInUp part-detail-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 380px', gap: '1.5rem', alignItems: 'start' }}>
            ...
            <div style={{ position: 'relative', background: '#F5F5F5', borderRadius: '20px', overflow: 'hidden', marginBottom: '1rem', border: '1px solid #EEE', boxShadow: '0 8px 30px rgba(0,0,0,0.03)', minHeight: 300, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: isVideoUrl(images[activeImg]) ? 'default' : 'crosshair' }}
              ...
              {effectiveStock === 0 && !isCheckingPincode && (
                <div style={{ position: 'absolute', inset: 0, background: 'rgba(255,255,255,0.6)', backdropFilter: 'blur(2px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 10 }}>
                  <span style={{ background: '#1567D3', color: 'white', fontWeight: 950, padding: '0.7rem 2.5rem', borderRadius: '14px', fontSize: '0.95rem', transform: 'rotate(-8deg)', border: '4px solid white', boxShadow: '0 12px 35px rgba(21, 103, 211, 0.25)', fontFamily: "'Space Grotesk', sans-serif", letterSpacing: '0.1em' }}>
              ...
              <div className="hide-scrollbar part-thumb-row" style={{ display: 'flex', gap: '0.8rem', overflowX: 'auto', paddingBottom: '0.8rem', marginBottom: '2rem' }}>
                {images.map((src, i) => (
                  <button key={i} onClick={() => { setActiveImg(i); setZoomed(false); }}
                    style={{ flexShrink: 0, width: 85, height: 85, borderRadius: '18px', overflow: 'hidden', border: '3.5px solid', borderColor: activeImg === i ? '#1567D3' : 'transparent', cursor: 'pointer', padding: 0, position: 'relative', transition: 'all 0.3s', background: '#F5F5F5', opacity: activeImg === i ? 1 : 0.7, boxShadow: activeImg === i ? '0 8px 20px rgba(21, 103, 211, 0.2)' : 'none' }}
            ...
            {part.description && (
              <div style={{ marginTop: '0.8rem', background: '#FFF', border: '1px solid #EEE', borderRadius: '20px', padding: '1.2rem', boxShadow: '0 4px 15px rgba(0,0,0,0.02)' }}>
                <h3 style={{ color: '#0F172A', fontFamily: "'Space Grotesk', sans-serif", fontSize: '1.4rem', fontWeight: 950, marginBottom: '0.8rem', display: 'flex', alignItems: 'center', gap: '0.6rem' }}>
                  <div style={{ width: 6, height: 24, background: '#1567D3', borderRadius: '4px' }} />
                  Component Overview
                </h3>
          ...
          <div style={{ position: 'sticky', top: 100 }}>
            ...
            <div style={{ marginBottom: '0.3rem' }}>
              <h1 className="text-2xl lg:text-3xl font-extrabold font-rajdhani tracking-tight leading-tight" style={{ color: '#1567D3', fontWeight: 800 }}>
                {part.name}
              </h1>
            </div>
                  ...
                  {availableSizes.map((variant, index) => {
                    ...
                    return (
                      <button
                        ...
                        style={{ height: 36, padding: '0 1rem', borderRadius: '8px', border: '1px solid', fontWeight: 700, fontSize: '0.7rem', transition: 'all 0.2s', cursor: oos ? 'not-allowed' : 'pointer', opacity: oos ? 0.4 : 1, background: isSelected ? '#EFF6FF' : '#FFF', borderColor: isSelected ? '#6FD8FF' : '#D1D5DB', color: isSelected ? '#172554' : '#111' }}
                      >
                        {variant.size}
                      </button>
                    );
              ...
              {cartItem ? (
                <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', background: '#F8FAFC', padding: '1rem', borderRadius: '14px', border: '1px solid #E2E8F0' }}>
                  <button onClick={() => updateQty(id, cartItem.quantity - 1)}
                    style={{ width: 44, height: 44, borderRadius: '12px', border: 'none', background: '#1567D3', color: 'white', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', fontSize: '1.4rem', fontWeight: 950 }}>-</button>
                 ...
                 <button disabled={isUnavailable || effectiveStock === 0} onClick={handleAddToCart}
                  className="btn-primary" style={{ width: '100%', height: '60px', borderRadius: '14px', fontSize: '1.1rem', fontWeight: 950, background: isUnavailable || effectiveStock === 0 ? '#E2E8F0' : '#1567D3', color: isUnavailable || effectiveStock === 0 ? '#94A3B8' : 'white', cursor: isUnavailable || effectiveStock === 0 ? 'not-allowed' : 'pointer', justifyContent: 'center', fontFamily: "'Space Grotesk', sans-serif", letterSpacing: '0.1em', boxShadow: isUnavailable || effectiveStock === 0 ? 'none' : '0 12px 30px rgba(21, 103, 211, 0.2)' }}>
        ...
        {similar.length > 0 && (
          <div style={{ marginTop: '4rem', marginBottom: '4rem', borderTop: '1px solid #EEE', paddingTop: '3rem' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
              <h3 style={{ fontFamily: "'Space Grotesk', sans-serif", fontSize: '2rem', fontWeight: 950, color: '#0F172A', letterSpacing: '0.02em' }}>RECOMMENDED <span style={{ color: '#1567D3' }}>SPARES</span></h3>
              <Link to="/parts" style={{ color: '#1567D3', fontWeight: 900, fontSize: '0.95rem', textDecoration: 'none', fontFamily: "'Space Grotesk', sans-serif", letterSpacing: '0.05em' }}>EXPLORE ALL</Link>
            </div>
